# AC215 | Milestone 3 | VeritasTrial
...
We now compare retrieval via our embedding model, versus the fuzzy string search by [ClinicalTrials.gov](https://clinicaltrials.gov/). Consider this search query:

> Find trials about the effect of drugs mainly used for high cholesterol on flu treatment.

If we fully input this query to [ClinicalTrials.gov](https://clinicaltrials.gov/), we get no search results (see [this search page](https://clinicaltrials.gov/search?term=Find%20trials%20about%20the%20effect%20of%20drugs%20mainly%20used%20for%20high%20cholesterol%20on%20flu%20treatment)). If we instead input the keywords *flu* and *high cholesterol*, it gave only one result, but is not yet recruiting which is out of scope of our consideration (see [this search page](https://clinicaltrials.gov/search?cond=flu,%20high%20cholesterol)). On the other hand, our model was able to find the following as the top match:

> Statin Therapy in Acute Influenza (NCT02056340)

Note how it was semantically related to flu (*influenza*) and high cholesterol (*statin*, which is a group of medicine that can help lower LDL cholesterol level in blood). This is what fuzzy string matching cannot achieve.
...
Here is an example, continuing from the example in the [embedding model](#embedding-model) section:

> **Query:** Summarize the outcome measures.
>
> **Bot:** The primary outcome measure was the change in inflammatory markers (IL-6) from baseline to 72 hours, assessed using a linear mixed effects model.  Secondary outcome measures included the severity of illness score from baseline to 72 hours, assessed using a composite score of 5 major symptoms. Other outcomes included progression to shock state, severity of illness using APACHE II scores, in-hospital mortality, and ICU and hospital length of stay.
